...
class FlutterMarqueeItem extends StatefulWidget {
  ...
  FlutterMarqueeItem({
    ...
    bool singleLine,
        ...
        this.textColor = textColor ?? Color(0xFFCCCCCC),
...
class WalkthroughState extends State<FlutterMarqueeItem> with SingleTickerProviderStateMixin {
  Animation animation;
  ...
  void initState() {
    ...
    _updateListener = () {
      setState(() {
        if (widget.modeListener.value) {
          transformAnimation = outTween.animate(CurvedAnimation(parent: animationController, curve: Curves.easeInOutExpo));
    ...
    animationController = AnimationController(vsync: this, duration: Duration(milliseconds: widget.itemDuration))..addListener(() {});

    transformAnimation = inTween.animate(CurvedAnimation(parent: animationController, curve: Curves.fastLinearToSlowEaseIn))..addListener(() => setState(() {}));
  ...
  Widget build(BuildContext context) {
    Widget current;
    Matrix4 transform = isXAniamation ? new Matrix4.translationValues(transformAnimation.value, 0, 0) : new Matrix4.translationValues(0, transformAnimation.value, 0);
